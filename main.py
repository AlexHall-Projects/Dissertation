...
class MainWindow(qtw.QMainWindow):
    ...
    def test_concluded_msg(self, cam_lag):
        ...
        # into separate function due to status bar buggy output

    "function for viewing the video files"
    def vid_file(self):
        os.system(
            'explorer.exe "C:\\Users\\ajh-1\\Video_file"')  # explorer opens on documents rather than given file
        # path, unsure why.
    ...
    def capture_duration(self, item):  # receives file list item from file_list
        fn = item.text()  # extract text data of file, ie file name
        url = qtc.QUrl.fromLocalFile(self.video_dir.filePath(fn))  # get URL of file
        print(url)  # unfortunately PyQt5 doesn't give a clean file path to hand to openCV
